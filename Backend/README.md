...
| `DECIDE_FREE_DEEP_DECISIONS_PER_MONTH` | How many "complex" decisions one install may spend without a verified Pro transaction, per calendar month, beyond the one always-free first decision (`src/quota.ts`). Defaults to 0, mirroring `FeatureAccess.freeDeepDecisionsPerMonth` in `App/App/AppEnvironment.swift` — the two aren't wired together, so keep them in sync by hand if either changes. Set to 0 on purpose: a recurring free allowance is something a casual user can settle into permanently with no pressure to convert. The App Store's introductory free trial on the Pro subscription is the real try-before-you-buy mechanism now — it has a natural end and converts to a real subscription unless cancelled. |
